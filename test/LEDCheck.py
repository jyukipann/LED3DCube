...
def strip2mat(strip, mat5):
	...
	for z in range(5):
		for y in range(5):
			for x in range(5):
				strip.setPixelColor(int(m2s[z,y,x]), Color(int(mat5[x,y,z,0]),int(mat5[x,y,z,1]),int(mat5[x,y,z,2])))
	strip.show()
...
def flying_bee(strip,colors,wait_ms=500):
	...
	mat5 = np.zeros((5,5,5,3),dtype=int)
	b = [15]*3
	mat5[0:1,:,0:1] = np.array(b,dtype=int)
...
if __name__ == '__main__':
	...
	try:
		while True:
			flying_bee(strip,[Color(244,213,0),Color(0,0,255),Color(255,165,0),Color(255,0,0),Color(0,255,0)])
			"""
